    ...
    const { data: prenotazioniUtente } = useQuery({
        ...
        queryFn: async () => {
            return await fetch('http://localhost:4321/prenotazione/prenotazioniutente/' + localStorage.getItem("userId"), {
                ...
                if (data.statusCode == 403 || data.statusCode == 500 || data.statusCode == 401) {
                    localStorage.clear();
                    navigate('/');
                } else {
    ...
    return (
        <>
            ...
            <div className="mt-24 mx-auto w-8/12 p-6 border border-gray-200 rounded-lg shadow bg-gray-800 border-gray-700 text-white">
                ...
                {
                    prenotazioniUtente?.map((pre, index) => {
                        return (
                            <>
                                <div className="w-auto border boder-solid rounded-md border-gray-500 mb-8 p-4" key={"btn-"+index}>
                                    ...
                                    <p className="text-white-700">{pre.ristorante.indirizzo}, prenotato per la fascia oraria <span className="text-indigo-500">{pre.fasciaOraria}</span> - tavolo prenotato per: <span className="text-emerald-500">{pre.numeroPersone}</span></p>
                                </div>
                            </>
                        )
                    })
